    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                // TODO: get miner reward from first output of coinbase tx
                if (block.major_version && block.major_version >= 6) {
                    log('info', logSystem, 'Block %d BitTube reward distribution share %d percent: %d', [block.height, 60, Math.round(block.reward * 0.6)]);
                    feePercent += 0.6;
                } else {
