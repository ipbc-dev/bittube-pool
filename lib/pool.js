...
var hopping_penalties = {}; // miner -> [expires, penalty]
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            if (config.poolServer.antiHopping && config.poolServer.antiHopping.enabled) {
                if (difficulty >= config.poolServer.antiHopping.triggerDiff) {
                    redisClient.hmget(config.coin + ':workers:' + miner.login, 'lastShare', 'hoppingPenaltyExpiry', function(err, replies) {
                        if (err){
                            log('error', logSystem, 'Failed to retrieve last share hopping data: %j', [err]);
                            return;
                        }
                        var now = Date.now() / 1000 | 0;
                        if (!replies[0] || parseInt(replies[0]) < now - 60*config.poolServer.antiHopping.triggerMinutes) {
                            log('info', logSystem, 'Miner %s triggered hopping penalty', [miner.login]);
                            var expiry = now + 60*config.poolServer.antiHopping.penaltyMinutes;
                            hopping_penalties[miner.login] = [expiry, config.poolServer.antiHopping.penaltyPercent / 100];
                            redisClient.hset(config.coin + ':workers:' + miner.login, 'hoppingPenaltyExpiry', expiry,
                                function(err, r) { if (err) log('error', logSystem, 'Failed to store penalty expiry for %s', [miner.login]); });
                        } else if (replies[1]) {
                            var expiry = parseInt(replies[1]);
                            if (expiry > now) {
                                hopping_penalties[miner.login] = [expiry, config.poolServer.antiHopping.penaltyPercent / 100];
                                log('info', logSystem, 'Miner %s has an active hopping penalty', [miner.login]);
                            }
                        }
                    });
                }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var penalized = 1.0;
    if (miner.login in hopping_penalties) {
        var p = hopping_penalties[miner.login];
        if (p[0] < dateNowSeconds) {
            delete hopping_penalties[miner.login];
            log('info', logSystem, 'Hopping penalty removed for %s', [miner.login]);
        } else {
            penalized -= p[1];
            if (penalized < 0) penalized = 0.0;
            log('info', logSystem, 'Penalizing score to %s%% for pool hopping for %s', [penalized * 100, miner.login]);
        }
    }
    ...
    if (slushMiningEnabled) {
        ...
        updateScore = ['eval', `
            ...
            /* args */ miner.login, job.difficulty * penalized, Date.now(), config.poolServer.slushMining.weight];
    }
    else {
        job.score = job.difficulty * penalized;
